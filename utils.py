...
def compute_chemberta(smiles_list, names, cache_name):
    """Compute ChemBERTa-77M-MLM embeddings (384-d CLS token).
    Returns {name: np.array(384,)}. Caches to EMBED_DIR/chemberta_{cache_name}.pt.
    """
    cache_path = os.path.join(EMBED_DIR, f"chemberta_{cache_name}.pt")
    if os.path.exists(cache_path):
        print(f"Loading cached ChemBERTa ({cache_name})")
        return torch.load(cache_path, map_location="cpu", weights_only=False)
    ...
    embed_dict = {}
    BS = 64
    print(f"Extracting ChemBERTa for {len(u_names)} ligands...")
    for i in range(0, len(u_names), BS):
        ...
        with torch.no_grad():
            out = model(**inputs)
        # Use CLS token directly — the pooler layer is not in the MLM checkpoint
        # (would be randomly initialized, producing garbage projections).
...
class LayerResidueDS(Dataset):
    """All-layer + per-residue + ligand."""
    def __init__(self, prot_names, lig_names, targets,
                 layer_dict, res_dict, lig_dict, num_layers):
        self.pn, self.ln, self.t = prot_names, lig_names, targets
        self.lyd, self.rd, self.ld, self.nl = layer_dict, res_dict, lig_dict, num_layers
    def __len__(self): return len(self.t)
    def __getitem__(self, idx):
        layers = torch.stack([self.lyd[self.pn[idx]][l] for l in range(self.nl)])
...
def train_loop(model, train_loader, val_loader, forward_fn,
    ...
    """Generic training loop. forward_fn(model, batch) -> (pred, target)."""
